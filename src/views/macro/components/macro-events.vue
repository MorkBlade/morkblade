<template>
  <div class="macro-events-container">
    <h3>{{ $t('macroEventsV1.macroEventSetting') }}</h3>
    <template v-if="itemData?.keyType === 'key'">
      <div class="outer-box">
        ...
        <input
          type="number"
          :value="parseFloat(itemData?.timeDifference).toFixed(2)"
          :style="{ margin: `${scaleValue(10)}px 0 ${scaleValue(20)}px 0` }"
          @input="changeDelayVal"
          @blur="verifyDelay"
        />
        <p>{{ $t('macroEventsV1.changeStatus') }}:</p>
        <div class="btn-group">
          <div
          ...
          >
            ...
            <span>{{ $t('macroEventsV1.down') }}</span>
          </div>
    ...
    <template v-else>
      <div class="outer-box">
        <p :style="{ marginTop: `${scaleValue(50)}px` }">{{ $t('macroEventsV1.changeDelayTip') }}</p>
  ...
  <div class="character-container" v-if="showCharacter">
    <div class="content">
      <h3>{{ $t('macroEventsV1.changeKeyTip') }}:</h3>
      ...
      <div class="operation-btn">
        <saveConfigBtn :btnText="$t('macroEventsV1.changeApply')" :icon="icon1" :needKeys="false" @saveConfig="saveConfig" :verify="true" />
        <saveConfigBtn
          :btnText="$t('macroEventsV1.changeCancel')"
          :icon="icon2"
          :needKeys="false"
          :verify="true"
          type="warning"
...
const characterArr = computed(() => [
  { name: t('macroEventsV1.basic'), icon1: basicIcon1, icon2: basicIcon2 },
  { name: t('macroEventsV1.extend'), icon1: extendIcon1, icon2: extendIcon2 },
  { name: t('macroEventsV1.special'), icon1: specialIcon1, icon2: specialIcon2 },
  { name: t('macroEventsV1.keyboard'), icon1: keyboardIcon1, icon2: keyboardIcon2 },
  { name: t('macroEventsV1.mouse'), icon1: mouseIcon1, icon2: mouseIcon2 },
]);
const extend = [
