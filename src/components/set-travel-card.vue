...
const handleBlur = (e) => {
  ...
  if (isVersion2) {
    if (inputValue <= 0.001 && !deadZone) showMessage(t('setTravelCard.minValue2'), 'warning');
  } else {
    if (inputValue <= 0.005 && !deadZone) showMessage(t('setTravelCard.minValue1'), 'warning');
  }
  if (inputValue >= max) showMessage(t('setTravelCard.maxValue'), 'warning');
