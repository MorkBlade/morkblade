...
watch(
  ...
  () => {
    ...
    if (macroData && macroData.data) curMacro.value = [...macroData.data];
    // curMacro.value = data;
  },
...
const onClick = (idx) => {
  ...
  switch (idx) {
    ...
    case 2: // 插入事件
      ...
      const lastKeyEvent = [...curMacro.value].reverse().find((item) => item.keyType === 'key');
...
const updateMacroItemKey = (newKey) => {
  ...
  if (curMacro.value[selectedItemIndex.value]) {
    curMacro.value[selectedItemIndex.value].key = getKeysByValue(newKey);
...
const updateMacroItemDelay = (newDelay) => {
  ...
  // 更新内部数组中的项
  if (curMacro.value[selectedItemIndex.value]) {
    curMacro.value[selectedItemIndex.value].timeDifference = newDelay;
...
// 宏类型设置，从 macroData 获取初始值
const macroTypeSettings = computed(() => {
  // 查找当前选中宏的类型设置
  if (macroData) {
    return {
      mode: macroData.mode || 0,
      repeatCount: macroData.repeatCount || 1,
      repeatInterval: macroData.repeatInterval || 1,
    };
  }
  // 默认设置
  return {
